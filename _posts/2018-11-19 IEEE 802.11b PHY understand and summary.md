...
&emsp;&emsp;假如要使用 11 Mb/s 的速率传输 1024 us长度的数据，且发送频率与符号时钟同源，那么帧头的 SIGNAL、SERVICE、LENGTH 的值应该是：
...
&emsp;&emsp;需要注意的是，第一，在 11b 协议中做 CRC-16 计算时时根据发送数据位顺序来计算的，因为全是低位在前发送，所以计算时都是低位开始做 CRC-16；第二，在 11b 协议中用于做 CRC-16 计算的寄存器初值是全一，即 0xFFFF，而非常规的全零。第三，在做完 CRC-16 计算之后需要再与 0xFFFF 做一次模二除法运算，即取反。

&emsp;&emsp;以下是对 2.3.2.7 short PPDU header用例 CRC-16 计算的仿真效果：
